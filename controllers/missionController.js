...
export const createMission = async(req, res, next) => {
    ...
    try {
        const mission = new Mission({ name, dateBegin, dateEnding});
        const personal = await Personal.findById(owner);
        personal.mission.push(mission);
        await personal.save();
...
export const deleteMission = async(req, res, next) => {
    ...
    try {
        const mission = await Mission.findById(id);
        const personal = await Personal.findById(mission.owner);

        personal.mission.pull(mission);
        await personal.save();
